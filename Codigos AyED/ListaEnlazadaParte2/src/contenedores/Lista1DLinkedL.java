...
public abstract class Lista1DLinkedL extends Lista0DLinkedL implements OperacionesCL3 {
	...
	public void insertar(Object elemento, int posicion) {
		NodoDoble nodo; //nodo auxiliar
		//Primero verificamos que la posicion sea valida
		if (posicion > tamanio() || posicion < 0) {
			...
			if (posicion == 0) { // insercion al comienzo
				...
				if (posicion == tamanio()) { // insercion al final
					this.finalL = new NodoDoble(elemento, this.finalL, null); // nuevo nodo final
					this.finalL.getPrevNodo().setNextNodo(this.finalL); // reconexion penultimo nodo al nuevo final
				}else{
					// insercion al medio					
					NodoDoble prev, next; //nodos auxiliares para realizar la busqueda
					prev = this.frenteL;
